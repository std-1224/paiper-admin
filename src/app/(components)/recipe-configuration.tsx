...
export default function RecipeConfiguration() {
	...
	const filteredRecipes = recipesData.filter((recipe) => recipe.name.toLowerCase().includes(searchTerm.toLowerCase()));
	...
	const addNewIngredient = () => {
		setNewIngredients([...newIngredients, { name: ingredientName, quantity: quantity, unit: unit }]);
		setIngredientName('');
		setQuantity('');
		setUnit('ml');
	};
				...
				<div className='border rounded-lg p-4'>
					<div className='flex items-center gap-3 mb-3'>
						...
						<div>
							...
							<div className='text-2xl font-bold text-blue-500'>{recipesData.length}</div>
						</div>
					</div>
				</div>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
						{filteredRecipes.map((recipe) => (
							<div
								...
								{recipe.stock !== null ? (
									<Badge className={cn('mb-3', recipe.stock > 0 ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200')}>
								...
								{recipe.stock === null && <div className='mt-3 p-2 bg-red-50 text-red-700 text-sm rounded border border-red-200'>Falta stock de ingredientes</div>}
							</div>
						))}
					</div>
				</>
						...
						<div className='border-t pt-4' >
							...
							<div className='grid grid-cols-12 gap-2'>
								<div className='col-span-5'>
									...
									<Select value={ingredientName} onValueChange={(value) => setIngredientName(value)}>
										<SelectTrigger>
						...
						<Button onClick={selectedRecipe ? updateRecipe : addNewRecipe}>
							{
								...
								<div className='flex items-center'>
									...
									Procesando...
								</div>:(selectedRecipe ? 'Actualizar Receta' : 'Crear Receta')
							}
						</Button>
					</DialogFooter>
				</DialogContent>
